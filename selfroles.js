...
    } catch (error) {
        console.error("CRITICAL ERROR: Could not load roles.json. Self-roles disabled. Error:", error.message);
        // Setting rolesConfig to an empty object to prevent later crashes if file read failed
        rolesConfig = {}; 
    }
}
...
async function createRolesPanel(message) {
    // CRITICAL CHECK: Ensure config is loaded before trying to access properties
    if (!rolesConfig || Object.keys(rolesConfig).length === 0) {
        return message.channel.send("Error: Role configuration is empty. Check bot logs for roles.json errors.")
...
    );
    
    // 3. Send the message to the CURRENT channel (message.channel)
    try {
        await message.channel.send({ embeds: [roleEmbed], components: [row] });
        ...
        console.error(`ERROR: Failed to post roles panel in channel ${message.channel.name}. Check bot permissions (Send Messages, Embed Links).`);
        console.error(error); 
    }
}
